...
- 开发模式完成校验后会存草稿并二次确认，不会自动发布；正式模式会在检查通过后自动发布。
...
- 视频封面会先在视频所在文件夹按文件名匹配；没有匹配图片才从 `video-covers` 中挑选。
...
- 开发模式只有看到顶部【保存成功】、等待 3 秒后跳转内容管理并精确找到标题，才会把已用 `.mp4` 移到 `used`；正式模式则在确认真的发布成功后移动。
...
```powershell
git clone https://github.com/Scope-lxy/lxy-skills.git "<临时目录>\lxy-skills"
Copy-Item "<临时目录>\lxy-skills\ixBrowser_qq_publish" "<你的skill目录>\ixBrowser_qq_publish" -Recurse

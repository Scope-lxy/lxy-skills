...
- 发布前不再区分开发模式和正式模式。完成最终校验后，统一点击【存草稿】；按钮按【存草稿】、【保存草稿】、【草稿】的顺序精确匹配。
...
- 文章封面上传后必须确认封面预览发生变化；如果预览不变，要直接报“文章封面上传后预览未变化”，不能继续存草稿。
- 点击存草稿后，必须以页面顶部出现的【保存成功】提示作为第一重成功信号，不能使用编辑器左下角的【已保存】自动保存提示。随后等待 `3` 秒，主动跳转到内容管理页，精确确认列表中存在当前标题，作为第二重成功信号；任一确认失败都不能移动视频。
- 配图确认、标题回读、封面上传、声明确认、存草稿按钮和保存成功提示等非上传关键步骤后要短暂停顿，让页面状态先落稳再继续；视频上传开始后不能通过刷新页面或重跑命令来“重试”。
- CLI 在长等待阶段要持续输出明确进度，例如视频上传已开始、标题和封面已设置、正在等待上传完成，以及“视频上传进度 xx% / 已等待 xx 秒，继续等待，不要结束任务”，避免外层 agent 因静默而误判流程已结束。
- 只有顶部【保存成功】提示和内容管理页标题复核都通过后，才把已用 `.mp4` 移动到 `used`；失败或未确认时不移动视频。
